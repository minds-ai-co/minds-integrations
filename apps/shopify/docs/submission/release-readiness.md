...
| Billing | Owner selected free install with Minds-account billing (2026-09-15). Pilot Studies are already created as the linked Minds user and use that account's existing response allowance | Shopify's written off-platform billing exception (requirement 1.2); app configured free in the Partner Dashboard with no Shopify charges; insufficient allowance blocks before execution; no double billing |
