...
| Privacy webhooks | Signed `app/uninstalled` handler only | Implement and subscribe all three mandatory privacy topics; verify signatures, duplicates, retries and completed deletion/export work |
| Data retention | Uninstall deletes Shopify links/challenges and connection snapshot references; canonical Studies remain in the consenting Minds account | Inventory copied product text/images in Studies, responses, queues, logs and backups; define and exercise lawful retention/deletion behavior before promising erasure |
