...
try{
 ...
 if(!database){
  ...
  let ready=false;
  for(let i=0;i<60;i++){
   try{execFileSync('docker',['exec',name,'pg_isready','-h','127.0.0.1','-U','postgres','-d','slack_test'],{stdio:'ignore'});ready=true;break}catch{await new Promise(r=>setTimeout(r,500))}
  }
